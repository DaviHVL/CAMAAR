...
RSpec.describe "Gerenciamento de Templates (exclusão)", type: :request do
  it "permite que o dono do template exclua o template" do
    ...
    follow_redirect!
    expect(response.body).to include("Template 'Para Deletar' excluído com sucesso.")
  ...
  it "não permite que outro usuário exclua o template" do
    ...
    follow_redirect!
    expect(response.body).to include("Você não tem permissão para excluir este template.")
